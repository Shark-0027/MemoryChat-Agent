"""Settings page for the MemoryChat-Agent UI.

Renders a read-only view of the application's configuration: the active
LLM provider/model/base URL, embedding model, memory backend, storage
paths, and application metadata.
...
and restarts the app. The API key is never shown in full.
"""
...
def render_settings_page() -> None:
    ...
    Displays the LLM, embedding, memory backend, storage, and
    application configuration in grouped sections. Sensitive values
    (API keys) are masked.
    """
